...
const formContainers = document.querySelectorAll(".container__form");

// Обрабатываем каждый контейнер формы
formContainers.forEach((container) => {
  const slides = container.querySelector(".slides");
  const dots = container.querySelectorAll(".pagination-dot");
  let currentSlide = 0;

  // Привязываем события к точкам пагинации для этого контейнера
  dots.forEach((dot, index) => {
    dot.addEventListener("click", () => {
      goToSlide(slides, dots, index);
    });
  });

  // Находим элементы выбора пола в этом контейнере
  const genderOptions = container.querySelectorAll(".gender-option");
  genderOptions.forEach((option) => {
    option.addEventListener("click", () => {
      selectGender(container, option.textContent.toLowerCase());
    });
  });
});

// Функция для переключения слайдов
function goToSlide(slidesElement, dotsElements, slideIndex) {
  slidesElement.style.transform = `translateX(-${slideIndex * 50}%)`;
...
function selectGender(container, gender) {
...
loginButtons.forEach((button, index) => {
  button.addEventListener("click", () => {
    ...
    const targetModal = button.dataset.modalTarget
      ? document.querySelector(button.dataset.modalTarget)
      : modals[index % modals.length];
...
// Добавление обработчика для переключения видимости пароля
...
// Add click event to gender options
genderOptions.forEach((option) => {
  option.addEventListener("click", () => {
...
// Initialize the pagination dots functionality
